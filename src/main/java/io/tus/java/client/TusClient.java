...
public class TusClient {
    ...
    private int upload_url_lenth;
    private URL downloadCreationURL;
    ...
    public void setUploadCreationURL(URL uploadCreationURL) {
        ...
        upload_url_lenth = uploadCreationURL.toString().length() + 1;
    }
    
    public void setDownloadCreationURL(URL downloadCreationURL) {
        this.downloadCreationURL = downloadCreationURL;
    }
    ...
    public URL getDownloadCreationURL() {
        return downloadCreationURL;
    }
    ...
    public TusDownloader createDownload(TusDownload download) throws ProtocolException, IOException {
        URL downloadURL = download.getDownloadURL();
        
        if(resumingEnabled) {
            urlStore.set(download.getFingerprint(), downloadURL);
        }
        
    	return new TusDownloader(this, downloadURL, download.getSaveFilePath(), 0);
    }
    ...
    public TusUploader createUpload(@NotNull TusUpload upload) throws ProtocolException, IOException {
        ...
        //将本地文件名和服务器文件名的映射存在客户端
        //String server_file_name = urlStr.substring(upload_url_lenth);
        Properties properties = new Properties();
        properties.load(new FileInputStream("F:\\Workspace_for_jee\\tus-java-client-master\\config.properties"));
        properties.setProperty(upload.getLocalFilePath(), urlStr);
        properties.store(new FileOutputStream("F:\\Workspace_for_jee\\tus-java-client-master\\config.properties"), "TUS本地服务器文件映射"); 
        ...
        if(resumingEnabled) {
            urlStore.set(upload.getLocalFilePath(), uploadURL);
        }

        return new TusUploader(this, upload.getLocalFilePath(), uploadURL, upload.getTusInputStream(), upload.getSize(), 0);
    }
    
    
    public TusDownloader resumeDownload(@NotNull TusDownload download) throws FingerprintNotFoundException, ResumingNotEnabledException, ProtocolException, IOException {
    	if(!resumingEnabled) {
            throw new ResumingNotEnabledException();
        }
    	
    	//加载本地download_offset.properties文件，查看服务器上完整的文件及其在本地的offset
    	Properties properties = new Properties();
        properties.load(new FileInputStream("F:\\Workspace_for_jee\\tus-java-client-master\\download_offset.properties"));
    	if(properties.get(download.getSaveFilePath()) != null) {
    		urlStore.set(download.getSaveFilePath(), download.getDownloadURL());
    	}
    	String offsetStr = properties.getProperty(download.getSaveFilePath());
    	
    	URL downloadURL = urlStore.get(download.getSaveFilePath());
        if(downloadURL == null) {
            throw new FingerprintNotFoundException(download.getFingerprint());
        }
        
        //String offsetStr = download.getOffsetString();
        long offset = Long.parseLong(offsetStr);
        
    	return new TusDownloader(this, downloadURL, download.getSaveFilePath(), offset);
    }
    ...
    public TusUploader resumeUpload(@NotNull TusUpload upload) throws FingerprintNotFoundException, ResumingNotEnabledException, ProtocolException, IOException {
        ...
        //加载本地config.properties文件，查看本地文件路径和服务器文件路径映射
        Properties properties = new Properties();
        properties.load(new FileInputStream("F:\\Workspace_for_jee\\tus-java-client-master\\config.properties"));
        if (properties.getProperty(upload.getLocalFilePath()) != null) {
        	urlStore.set(upload.getLocalFilePath(), new URL(properties.getProperty(upload.getLocalFilePath())));
        }
        
        URL uploadURL = urlStore.get(upload.getLocalFilePath());
        if(uploadURL == null) {
        ...
        return new TusUploader(this, upload.getLocalFilePath(), uploadURL, upload.getTusInputStream(), upload.getSize(), offset);
    }

    
    
    public TusDownloader resumeOrCreateDownload(@NotNull TusDownload download) throws ProtocolException, IOException {
        try {
            return resumeDownload(download);
